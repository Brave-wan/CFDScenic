...
public class ImageCycleView extends LinearLayout {
	 ...
	 */
	public void setImageResources(List<CarouselImgBean.DataBean> infoList, ImageCycleViewListener imageCycleViewListener) {
		// 清除所有子视图
		...
		for (int i = 0; i < imageCount; i++) {
			...
			if (i == 0) {
				mImageViews[i].setBackgroundResource(R.mipmap.xuanzhong);
			} else {
				mImageViews[i].setBackgroundResource(R.mipmap.weixuanzhong);
			}
	...
	private final class GuidePageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int index) {
			...
			mImageIndex = index;
			index -= 1;
			mImageViews[index].setBackgroundResource(R.mipmap.xuanzhong);
	...
	private class ImageCycleAdapter extends PagerAdapter {
		 ...
		 * 图片资源列表
		 */
		private List<CarouselImgBean.DataBean> mAdList = new ArrayList<CarouselImgBean.DataBean>();
		...
		public ImageCycleAdapter(Context context, List<CarouselImgBean.DataBean> adList, ImageCycleViewListener imageCycleViewListener) {
			mContext = context;
			mAdList = adList;
		...
		public Object instantiateItem(ViewGroup container, final int position) {
			String imageUrl = mAdList.get(position).getImg_url().trim();
